...
public class UserCoreController extends BaseController {
	...
	public Map<String,Object> updatePswd(String pswd,String newPswd){
		System.out.println("pswd==================="+pswd+','+newPswd);
		//根据当前登录的用户帐号 + 老密码，查询。
	...
	/**
	 * 获取当前登录的用户帐号 by chendf 2017/1/4
	 * @return
	 */
	...
	/**
	 * 新增跟进记录获取跟踪人信息，查询 email 和nickname
	 * @return
	 */
	@RequestMapping(value="findContactName")
	@ResponseBody
	public Map<String,Object> findContactName(){
		try {
			resultMap.put("status", 200);
			resultMap.put("message", "查询成功!");
			resultMap.put("record", userService.findContactName());
		} catch (Exception e) {
			resultMap.put("status", 500);
			resultMap.put("message", "查询失败!");
			LoggerUtils.fmtError(getClass(), e, "查询信息出错。[%s]");
		}
		return resultMap;
	}
